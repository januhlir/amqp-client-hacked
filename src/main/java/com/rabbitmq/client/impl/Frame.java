...
/**
 * Represents an AMQP wire-protocol frame, with frame type, channel number, and payload bytes. TODO:
 * make state private
 */
public class Frame {
	/** Frame type code */
	...
	/**
	 * Constructs a frame for output with a type and a channel number and a fresh accumulator
	 * waiting for payload.
	 */
	...
	/**
	 * Protected API - Factory method to instantiate a Frame by reading an AMQP-wire-protocol frame
	...
	public static Frame readFrom(DataInputStream is) throws IOException {
		...
		if (payloadSize < 1000000) { // FIXME: hack! jan.uhlir
			logger.debug("processing frame payloadSize=" + payloadSize);
		} else {
			logger.error("Payload too huge! payloadSize=" + payloadSize);
			logger.info("Payload too huge! payloadSize=" + payloadSize);
			...
			try {
				...
				logger.error("IOException when reading messages first " + payload.length + " bytes (less bytes then expected?)");
			}
			StringBuilder payloadAsHexString = new StringBuilder();
			for (int j = 0; j < payload.length; j++) {
				payloadAsHexString.append(String.format("%02X ", payload[j]));
			}
			logger.error("Payload first bytes: " + payloadAsHexString.toString());
			logger.error("Payload first bytes: " + new String(payload));
			logger.info("Payload first bytes: " + payloadAsHexString.toString());
			logger.info("Payload first bytes: " + new String(payload));
			throw new IOException("Payload too huge! payloadSize=" + payloadSize);
		}
